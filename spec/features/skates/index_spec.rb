...
RSpec.describe 'Skates#index' do
  ...
  it 'shows an upcoming skate date as a link that takes user to that skate' do
    skate = @rink.skates.create!(date: DateTime.now + 5)

    visit '/skates'

    click_link(skate.date.strftime('%a %b %d, %Y').to_s)
